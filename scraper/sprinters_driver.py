...
class SprintersDriver:
    ...
    def get_next_href(
        self, pagination_selector: str, pagination_xpath_child: str
    ) -> str:
        return self._find_child_element_by_xpath(
            self._find_element_by_xpath(pagination_selector), pagination_xpath_child
        ).get_attribute("href")

    def get_links_from_advertisements(
        self, js_objects, card_xpath_featured: str, card_xpath_href: str
    ):
        ox_links = []
        otd_links = []
        for js_object in js_objects:
            try:
                ...
                link_to_child_site = self._find_child_element_by_xpath(
                    js_object, card_xpath_href
                ).get_attribute("href")
